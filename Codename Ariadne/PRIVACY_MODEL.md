...
The verified Phase 3 aggregate covers parser isolation, malicious structures, restricted-value handling, contentless defaults, keyed fingerprints, provenance, graph suppression, durable idempotency/replay, decision-policy invariants, and cross-profile rejection through `0005_graph_edge_origins`. Its historical privacy scans passed, and all `0006`–`0008` packages retain their separate identities. The current 57-operation candidate passes its complete aggregate, a **440-candidate privacy scan**, frozen/staged workflows, and ad-hoc packaged normal/abrupt lifecycle under `dccaaa5…` staged, `74325b3…` packaged-sidecar, and `08491aa…` desktop identities. It includes exact-name/revision-confirmed whole-profile purge with secure deletion and vacuum. No historical artifact identity is relabelled. These local results do not approve production release use: selective retention, authorised connectors, external-provider governance, production signing/notarisation, and clean-machine validation remain incomplete.
